...
export function claimGoal(G: GalileoProjectGameState, player: Player, index: number): boolean {
  ...
  if (goalTracker.goal === 'Control4RobotsLevel6' && !validateControl4RobotsLevel6(player)) {
    ...
    goalTracker.goal === 'Control5RobotsWithDouble' &&
    !validateControl5RobotsWithDouble(player)
  ) {
    return false;
  } else if (
    goalTracker.goal === 'Control7RobotsSamecColor' &&
    !validateControl7RobotsSamecColor(player)
  ) {
    return false;
...
function validateControl7RobotsSamecColor(player: Player): boolean {
  ...
  return (
    all.filter((r) => r.track === 'Earth').length >= 7 ||
